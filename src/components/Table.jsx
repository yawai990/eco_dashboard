...
const Table = ({ colData, rowData, border, clickFun }) => {
  ...
  return (
    <table className={`w-full ${border && 'border border-stone-200'}`}>
      <thead className='w-full bg-pink-600 dark:text-white text-head-gray'>
        <tr>
          {
            colData?.map(col => (
              <th key={col.id} className={`${col.name === '#' && 'max-w-[8px]'} min-w-[100px] py-2 capitalize text-center text-white font-semibold`}>{col.name}</th>
            ))
          }
        </tr>
      </thead> 
      ...
      <tbody>
          {
            ...
            return  <tr key={row._id || idx } className='capitalize text-center text-[#2B4865] dark:text-white font-semibold border-b border-stone-200 hover:bg-stone-100 dark:hover:bg-zinc-700 cursor-pointer'
            ...
            >
              <td className='max-w-[10px]'>{idx+1}</td>
              {
                rowKey?.map((key,ind)=>(
                  key !== '_id' &&
                   <td key={`tablerow-${ind}`} className={`mb-2 py-2 dark:text-white`}>
                    
                    {
                      ...
                      src={`${row[key][0].path !== undefined ? 
                       ...
                       :
                      <span className={`inline-block text-gray dark:text-white
